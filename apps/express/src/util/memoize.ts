...
const YahooClient = async () => {
  const yahooIdP = await Issuer.discover(process.env.NX_YAHOO_OIDC_ISSUER_BASE_URL as string);
...
const GoogleClient = async () => {
  const googleIdP = await Issuer.discover(process.env.NX_GOOGLE_OIDC_ISSUER_BASE_URL as string);

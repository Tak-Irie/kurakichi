...
const invalidLongUsername = UserName.create({
  username: 'loooooooooooooooooooooooooooong',
});
...
describe('ドメインユーザー', () => {
  test('ユーサー作成成功', () => {
    ...
    expect(result.username).toBe('okName');
    expect(result.email).toBe('success@email.com');
    // isHashedを確認
  ...
  test('不正なパスワード', () => {
    expect(invalidPassword.getErrorValue()).toBe(

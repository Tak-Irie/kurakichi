...
export const InquiryInfiniteTable: VFC<InquiryInfiniteTableProps> = ({
  ...
  const [getInquiry, { data, loading, error }] = useGetInquiriesByOrgIdLazyQuery();
  ...
  useEffect(() => {
    if (data?.getInquiriesByOrgId.inquiries) {
      setInquiries((prev) => {
        return {
          loadedInquiries: prev.loadedInquiries.concat(data.getInquiriesByOrgId.inquiries),
  ...
  return (
    ...
    >
      <TableInquiry inquiries={inquiries.loadedInquiries} orgId={orgId} tableLabel="お問い合わせ" />
    </InfiniteScroll>
  );
};

export const InquiryInfiniteTableWithStatus: VFC<InquiryInfiniteTableWithStatusProps> = ({
  limit = 20,
  initialInquiries,
  orgId,
  status,

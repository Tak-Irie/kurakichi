  ...
  return (
    <div className="text-sm">
          ...
          <div className="grid grid-cols-2 gap-5 row-gap-8 lg:col-span-4 md:grid-cols-4">
            ...
            <FooterItems
              ...
              items={[
                ...
                { label: '情報セキュリティポリシー', url: '/info/information-security-policy' },
              ]}
            />
            <FooterItems
              label="ご意見"
              items={[
                ...
                { label: 'これまでのご意見と回答', url: '/info/answer-to-opinion' },
              ]}
            />
          </div>
        </div>
      ...
      <div className="flex justify-center py-5 border-t sm:flex-row">
        <p className="text-sm text-gray-700">© Copyright 2021 kurakichi. All rights reserved.</p>
      </div>
    </div>
  );
};
    ...
    <div>
      ...
      <ul className="mt-2 space-y-2">
        {items.map((item) => {
          return (
            <li key={item.label}>
              <LinkNextjs overwriteCSS="text-gray-700" url={item.url} labelOrElement={item.label} />
            </li>
          );
        })}
      </ul>

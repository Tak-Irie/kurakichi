...
const invalidPasswordFnc = UserPassword.create({ password: 'short', isHashed: false });
describe('ドメインユーザー', () => {
  test('ユーサー作成成功', async () => {
  ...
  test('不正なユーザーネーム', () => {
    ...
    expect(invalidLongUsername.getErrorValue()).toBe('ユーザー名は最大20文字です');
  });
  ...
  test('不正なパスワード', async () => {
    ...
    expect(invalidPass.getErrorValue()).toBe('パスワードは8文字以上に設定してください');
  });
});

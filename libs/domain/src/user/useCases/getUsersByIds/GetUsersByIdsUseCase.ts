...
type GetUsersByIdsResponse = Either<
  UsersNotExistsError | InvalidInputValueError | UnexpectedError | StoreConnectionError,
  Result<DTOUser[]>
>;
...
export class GetUsersByIdsUseCase
  ...
  public async execute(arg: GetUsersByIdsArg): Promise<GetUsersByIdsResponse> {
    try {
      ...
      const dbResult = await this.UsersRepo.getUsersByIds(ids.map((id) => id.getValue()));
